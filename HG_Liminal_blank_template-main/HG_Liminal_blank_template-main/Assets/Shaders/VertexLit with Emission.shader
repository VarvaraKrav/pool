Shader "Legacy Shaders/VertexLit with Emission" {
Properties {
    _Color ("Main Color", Color) = (1,1,1,1)
    _SpecColor ("Spec Color", Color) = (1,1,1,1)
    _EmissionColor ("Emissive Color", Color) = (0,0,0,0)  // Emission Color
    _EmissionMap ("Emission (RGB)", 2D) = "white" {}      // Emission Texture
...
SubShader {
    ...
    Pass {
        ...
        SetTexture [_EmissionMap] {          // Add Emission texture
    ...
    Pass {
        ...
        sampler2D _MainTex;
        sampler2D _EmissionMap;               // Emission texture
        fixed4 _Color;
        ...
        fixed4 frag(v2f IN) : SV_Target {
            ...
            fixed4 emission = tex2D(_EmissionMap, IN.uv2.xy) * _EmissionColor;  // Apply emissive texture and color
            col.rgb += emission.rgb;  // Add emissive color
