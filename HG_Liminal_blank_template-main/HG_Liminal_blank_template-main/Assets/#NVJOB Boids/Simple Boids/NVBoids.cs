...
public class NVBoids : MonoBehaviour
{
    ...
    void CreateBird()
    {
        ...
        for (int b = 0; b < birdsNum; b++)
        {
            ...
            //birdsTransform[b].localRotation = Quaternion.Euler(0, Random.value * 360, 0);  <<-- this is original

            // Set fixed initial rotation to face the positive Z-axis (toward the sun) 
            birdsTransform[b].localRotation = Quaternion.LookRotation(Vector3.forward); // <<-- changed to this by J.Nisi
